...
/* L1-17 + L2-09：Speed 两段灵敏度 scrub（对齐 OpenCut scrubRanges）——
 * 慢放区(0.01~1) 160px/单位精细调，快放区(1~5) 48px/单位粗调，跨段自动切换消耗像素。
 * 复用 scrub-session.js 的共享 ScrubSession + attachScrub（scrubAcrossRanges 抽到全局，免双份）。 */
...
function buildSpeedFields(c) {
  ...
  const spdField = new PropertyField({
    ...
    write: (el, v) => {
      ...
      if (inp && document.activeElement !== inp) inp.value = Math.round(v * 100) / 100;  // L1-17：scrub 期间 InteractionManager 锁 refresh，updateSpeedFields 不被触发
    ...
    bind: (fld) => {
      ...
      // L1-17：⏩ 图标 drag-to-scrub（复用共享 ScrubSession + attachScrub，消除 L2-09 内联双份）
      const ic = fld.el.querySelector(".ic.scrub-handle");
      if (ic) {
        attachScrub(ic, {
          getStart: () => parseFloat(inp.value) || 1,
          ranges: SPEED_SCRUB_RANGES,                       // 分段灵敏度（慢放精细/快放粗调）
          onScrub: (v) => { inp.value = Math.round(v * 100) / 100; fld._draft.onInput(String(v)); },          // 实时预览
          onScrubEnd: (v) => { inp.value = Math.round(v * 100) / 100; fld._draft.onInput(String(v)); fld._draft.onCommit(); },  // 一条 undo
          onCancel: (sv) => { inp.value = Math.round(sv * 100) / 100; fld._draft.onInput(String(sv)); },      // 回退起始值
        });
      }
    },
  });
